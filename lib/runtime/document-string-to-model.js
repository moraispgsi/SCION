...
SCModel.prototype.prepare = function(cb, executionContext, hostContext) {
    ...
    if (!executionContext instanceof VM){
        executionContext = new VM({executionContext});
    }
    ...
    Promise.all(scriptPromises).then(function resolved(scripts) {
        try {
            if (self.datamodel) {
                executionContext.run(self.datamodel);
            }

            for (let i = 0; i < scriptCount; i++) {
                self.rootScripts[i].compiled.run(executionContext);
            }

            let modelFn = executionContext.run(self.module);
